...
function addSampleData() {
  ...
  const courses = [
    ...
    { id: "SAMPLE-BATCH-FND-JUN", programId: "SAMPLE-PROGRAM-FOUNDATION", name: "Foundation Yoga - June 2026", start: "2026-06-01", end: "2026-06-03", seats: 4, hallId: "SAMPLE-HALL-LOTUS", hall: "Lotus Hall", teacher: "Shri Narayanan Iyer", eligibility: "Age 18 and above", status: "Completed" }
  ];
  ...
  const participants = [
    ...
    { id: "SAMPLE-PART-009", name: "Ishaan Varma", age: 30, gender: "Male", phone: "98840 20009", email: "ishaan.sample@example.com", address: "Tiruppur, Tamil Nadu", emergencyContact: "Divya - 98840 29909", photo: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&w=400&q=80", notes: "Confirmed but room not yet allotted.", registrations: [] },
    { id: "SAMPLE-PART-010", name: "Jaya Lakshmi", age: 44, gender: "Female", phone: "98840 20010", email: "jaya.sample@example.com", address: "Trichy, Tamil Nadu", emergencyContact: "Ramesh - 98840 29910", photo: "https://images.unsplash.com/photo-1607746882042-944635dfe10e?auto=format&fit=crop&w=400&q=80", notes: "Checked in sample stay for check-out workflow.", registrations: [] }
  ];
  ...
  const registrations = [
    ...
    ["SAMPLE-PART-009", sampleRegistration("SAMPLE-REG-012", "SAMPLE-BATCH-FND-JUN", { pricingCategory: "General", accommodationType: "Double Occupancy", checkinDate: "2026-06-01", checkoutDate: "2026-06-03", certificate: false, registeredOn: "2026-05-13", notes: "Confirmed accommodation request pending room allocation." }), "complete"],
    ["SAMPLE-PART-010", sampleRegistration("SAMPLE-REG-013", "SAMPLE-BATCH-FND-JUN", { pricingCategory: "General", accommodationType: "Dormitory", roomId: "SAMPLE-ROOM-A101", checkedIn: true, checkedOut: false, checkinDate: "2026-06-01", checkoutDate: "2026-06-03", certificate: false, registeredOn: "2026-05-14", notes: "Checked in; use Room Allotments to check out and mark room dirty." }), "late"]
  ];
...
function bindEvents() {
  ...
  $("#roomAllotmentContent").addEventListener("click", (event) => {
    const allotAction = event.target.closest("[data-room-allot-action]");
    if (allotAction) {
      const select = document.querySelector(`[data-room-allotment="${allotAction.dataset.roomAllotAction}"][data-registration-id="${allotAction.dataset.registrationId}"]`);
      if (!select?.value) {
        showToast("Select a room before allotting.");
        return;
      }
      allotRoomToRegistration(allotAction.dataset.roomAllotAction, allotAction.dataset.registrationId, select.value);
      return;
    }
    const clearAction = event.target.closest("[data-room-clear-action]");
    if (clearAction) {
      allotRoomToRegistration(clearAction.dataset.roomClearAction, clearAction.dataset.registrationId, "");
      return;
    }
    const stayAction = event.target.closest("[data-room-stay-action]");
    if (stayAction) {
